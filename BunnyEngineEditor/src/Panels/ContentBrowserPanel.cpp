...
namespace BE {
	...
	{
		m_DocumentIcon = Texture2D::Create("Resources/Icons/ContentBrowser/DocumentIcon.png");
		m_FolderIcon = Texture2D::Create("Resources/Icons/ContentBrowser/FolderIcon.png");
	}

	void ContentBrowserPanel::SetContext(DockSpace* context)
	{
		m_Context = context;
	}
	...
	void ContentBrowserPanel::OnImGuiRenderer() {
		...
		for (auto& directoryEntry : std::filesystem::directory_iterator(m_CurrentDirectory)) {
			...
			Ref<Texture2D> icon = directoryEntry.is_directory() ? m_FolderIcon : m_DocumentIcon;
